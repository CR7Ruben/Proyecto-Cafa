...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!username || !password || !tabToken) return res.status(400).json({ error: 'Missing fields (include tabToken)' });
    ...
    await Session.create({ id: sessionId, userId: user.id, tabToken, jwtId: jti, expiresAt });
